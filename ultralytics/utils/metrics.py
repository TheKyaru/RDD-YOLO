...
def bbox_iou(
    ...
    Supports IoU, GIoU, DIoU, CIoU, and SIoU.

    Args:
        box1 (torch.Tensor): bounding boxes (x, y, w, h) or (x1, y1, x2, y2)
        box2 (torch.Tensor): bounding boxes (x, y, w, h) or (x1, y1, x2, y2)
        xywh (bool, optional): If True, input boxes are in (x, y, w, h) format.
        GIoU (bool, optional): If True, calculate Generalized IoU.
        DIoU (bool, optional): If True, calculate Distance IoU.
        CIoU (bool, optional): If True, calculate Complete IoU.
        SIoU (bool, optional): If True, calculate Scylla IoU (https://arxiv.org/pdf/2205.12740.pdf).
        eps (float, optional): small value to avoid division by zero.

    Returns:
        torch.Tensor: IoU, GIoU, DIoU, CIoU, or SIoU
    """
    ...
    # Extended IoU variants
    if CIoU or DIoU or GIoU or SIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height

        if CIoU or DIoU or SIoU:  # Distance/Complete/Scylla IoU
            ...
            if CIoU:
                ...
                # Scylla IoU (https://arxiv.org/pdf/2205.12740.pdf)
                ...
                shape_cost = (1 - torch.exp(-omiga_w)) ** 4 + (1 - torch.exp(-omiga_h)) ** 4
